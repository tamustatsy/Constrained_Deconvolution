...
source("../../../PI_deconvUknownth4het.r")
source("../../../fdecUknownhet.r")
source("../../../phiK2.r")
source("../../../rlap.R")
source("../../../outerop.r")
source("../../../kernel_decon_known_u_het.R")
...
df.t <- 5
x <- rt(n, df = df.t)
b_lap <- sqrt((1 + x/4)^2/2)                                #the variance of laplace is same as variance of xs
...
res.kernel <- kernel_het(n = n, W = w, sigmaj = b_lap, xs = 11, lfg = 2*lfg, errortype = 'Lap')
...
write.table(data.frame(seed, imae_CB, imse_CB, imae_Kernel, imse_Kernel, x.grid, 
            ...
            file=paste("./result/", n, "/seed", seed, "_HET.csv", sep = ""), 
